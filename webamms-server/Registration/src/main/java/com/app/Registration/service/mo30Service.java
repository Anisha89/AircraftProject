...
import com.app.Registration.repository.mo30Repository;

@Service
@Component
public class mo30Service implements Iservice {
	@Autowired
	private mo30Repository repo;

	public List<mo30> fetchProductList() {
		return repo.findAll();
	}

	public mo30 saveservicetoDB(mo30 service) {
		return repo.save(service);
	}

	public java.util.Optional<mo30> fetchServiceById(int id) {
		return repo.findById(id);
	}

	public java.util.Optional<mo30> deleteserviceByIdtest(int id) throws Exception {
		java.util.Optional<mo30> u = this.repo.findById(id);
		if (u.isPresent())
	...
	public List<mo30> findPaginatedmo30(int pageNo, int perpage) {
		...
		Page<mo30> pagedresult = repo.findAll(pageable);
	...
	public List<mo30> findByservicenumber(String seqno, int pageable, int perpage) {
		List<mo30> pagedresult1 = (List<mo30>) repo.findByservice1(seqno, PageRequest.of(pageable, perpage));
		return pagedresult1;
	}

	public List<mo30> findByservicesmrdno(String seqno, int pageable, int perpage) {
		List<mo30> pagedresult1 = (List<mo30>) repo.findByservicesmrdno1(seqno, PageRequest.of(pageable, perpage));
		return pagedresult1;
	}
