...
import com.app.Registration.repository.ConcessionRepository;

@Service
@Component
public class ConcessionService implements Iservice {
	@Autowired
	private ConcessionRepository repo;

	public List<Concession> fetchProductList() {
		return (List<Concession>) repo.findAll();
	}

	public Concession saveConcessiontoDB(Concession Concession) {
		return repo.save(Concession);
	}

	public java.util.Optional<Concession> fetchConcessionById(int id) {
	...
	public String deleteProductById(int id) {
		...
		try {
			repo.deleteById(id);
			result = "Concession successfully Deleted";
		} catch (Exception e) {
			result = "Concession with id is not deleted";
		}
		return result;
	}

	public java.util.Optional<Concession> deleteConcessionByIdtest(int id) throws Exception {
		java.util.Optional<Concession> u = this.repo.findById(id);
		if (u.isPresent())
	...
	public List<Concession> findPaginatedconcession(int pageNo, int perpage) {
		...
		Pageable pageable = PageRequest.of(pageNo, perpage, Sort.by("idConcession").descending());
		Page<Concession> pagedresult = repo.findAll(pageable);
		return pagedresult.toList();
	}

	public List<Concession> findby(String aircraft) {
		List<Concession> pageresult3 = (List<Concession>) repo.findby(aircraft);
		return pageresult3;
	}
	...
	public List<Concession> findPaginated1(int pageNo, int perpage) {
		// TODO Auto-generated method stub
		Pageable pageable = PageRequest.of(pageNo, perpage, Sort.by("aircraft").descending());
		Page<Concession> pagedresult = repo.findAll(pageable);
		return pagedresult.toList();
	}
