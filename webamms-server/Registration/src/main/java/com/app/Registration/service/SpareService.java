...
import com.app.Registration.repository.SpareRepository;

@Service
@Component
public class SpareService implements Iservice {
	@Autowired
	private SpareRepository repo;

	public spare saveSpare(spare spare) {
		return repo.save(spare);
	}

	public List<spare> fetchProductListspare() {
		return repo.findAll();
	}

	public java.util.Optional<spare> fetchspareById(int id) {
	...
	public List<spare> findlist(String x) {
		List<String> names = Arrays.asList(x.split("\\s*,\\s*"));
		List<Integer> listOfInteger = convertStringListToIntList(names, Integer::parseInt);
		List<spare> employees = repo.findbyserviceno(listOfInteger);
		return employees;
	}

	public java.util.Optional<spare> deletespareByIdtest(int id) throws Exception {
		java.util.Optional<spare> u = this.repo.findById(id);
		if (u.isPresent())
	...
	public List<spare> findPaginatedspare(int pageNo, int perpage) {
		...
		Pageable pageable = PageRequest.of(pageNo, perpage, Sort.by("idspare").descending());
		Page<spare> pagedresult = repo.findAll(pageable);
	...
	public List<spare> findBysparenumber(String seqno, int pageable, int perpage) {
		List<spare> pagedresult1 = (List<spare>) repo.findByspare1(seqno, PageRequest.of(pageable, perpage));
		return pagedresult1;
	}

	public List<spare> findByspareodm(String seqno, int pageable, int perpage) {
		List<spare> pagedresult1 = (List<spare>) repo.findByspareodm1(seqno, PageRequest.of(pageable, perpage));
		return pagedresult1;
	}

	public int countspare(String seqno) {
		return repo.countspare(seqno);
	}
	...
	@Override
	public List<spare> findByspareodm(String seqno,
			org.springframework.boot.autoconfigure.data.web.SpringDataWebProperties.Pageable pageable) {
		// TODO Auto-generated method stub
		return null;
	}
