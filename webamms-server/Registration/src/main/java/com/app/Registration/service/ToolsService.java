...
import com.app.Registration.repository.ToolsRepository;

@Service
@Component
public class ToolsService implements Iservice {
	@Autowired
	...
	public Tools saveToolstoDB(Tools tools) {
		return repo.save(tools);
	}

	public java.util.Optional<Tools> fetchToolsById(int id) {
	...
	public List<Tools> findlist(String x) {
		List<String> names = Arrays.asList(x.split("\\s*,\\s*"));
		List<Integer> listOfInteger = convertStringListToIntList(names, Integer::parseInt);
		List<Tools> employees = repo.findbyservicetools(listOfInteger);
		return employees;
	}

	public java.util.Optional<Tools> deletetoolsByIdtest(int id) throws Exception {
		java.util.Optional<Tools> u = this.repo.findById(id);
		if (u.isPresent())
	...
	public List<Tools> findPaginatedtools(int pageNo, int perpage) {
		...
		Pageable pageable = PageRequest.of(pageNo, perpage, Sort.by("idtools").descending());
		Page<Tools> pagedresult = repo.findAll(pageable);
	...
	public List<Tools> findByToolsnumber(String toolsno, int pageable, int perpage) {
		List<Tools> pagedresult1 = (List<Tools>) repo.findBytoolsnumber1(toolsno, PageRequest.of(pageable, perpage));
		return pagedresult1;
	}

	public List<Tools> findBytoolsgdm(String toolsno, int pageable, int perpage) {
		List<Tools> pagedresult1 = (List<Tools>) repo.findBytoolsgdm1(toolsno, PageRequest.of(pageable, perpage));
		return pagedresult1;
	}

	public int counttools(String toolsno) {
		return repo.counttools(toolsno);
	}
