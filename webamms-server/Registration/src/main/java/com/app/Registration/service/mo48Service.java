...
import com.app.Registration.repository.mo48Repository;

@Service
@Component
public class mo48Service implements Iservice {
	@Autowired
	private mo48Repository repo;

	public List<mo48> fetchProductList() {
		return repo.findAll();
	}

	public mo48 saveservicetoDB(mo48 service) {
		return repo.save(service);
	}

	public java.util.Optional<mo48> fetchServiceById(int id) {
		return repo.findById(id);
	}

	public java.util.Optional<mo48> deleteserviceByIdtest(int id) throws Exception {
		java.util.Optional<mo48> u = this.repo.findById(id);
		if (u.isPresent())
	...
	public List<mo48> findPaginatedmo48(int pageNo, int perpage) {
		...
		Page<mo48> pagedresult = repo.findAll(pageable);
	...
	public List<mo48> findByservicenumber(String seqno, int pageable, int perpage) {
		List<mo48> pagedresult1 = (List<mo48>) repo.findByservice1(seqno, PageRequest.of(pageable, perpage));
		return pagedresult1;
	}

	public List<mo48> findByservicesmrdno(String seqno, int pageable, int perpage) {
		List<mo48> pagedresult1 = (List<mo48>) repo.findByservicesmrdno1(seqno, PageRequest.of(pageable, perpage));
		return pagedresult1;
	}
