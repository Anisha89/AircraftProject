...
import com.app.Registration.repository.GseRepository;

@Service
@Component
public class GseService implements Iservice {
	@Autowired
	private GseRepository repo;

	public List<Gse> fetchProductList() {
		return repo.findAll();
	}

	public Gse saveGsetoDB(Gse gse) {
		return repo.save(gse);
	}

	public java.util.Optional<Gse> fetchGseById(int id) {
		return repo.findById(id);
	}

	public java.util.Optional<Gse> deletegseByIdtest(int id) throws Exception {
		java.util.Optional<Gse> u = this.repo.findById(id);
		if (u.isPresent())
	...
	public List<Gse> findPaginatedgse(int pageNo, int perpage) {
		...
		Pageable pageable = PageRequest.of(pageNo, perpage, Sort.by("idgse").descending());
		Page<Gse> pagedresult = repo.findAll(pageable);
	...
	public List<Gse> findByGsenumber(String gseno, int pageable, int perpage) {
		List<Gse> pagedresult1 = (List<Gse>) repo.findBygsenumber1(gseno, PageRequest.of(pageable, perpage));
		return pagedresult1;
	}

	public List<Gse> findByGsegdm(String gseno, int pageable, int perpage) {
		List<Gse> pagedresult1 = (List<Gse>) repo.findBygsegdm1(gseno, PageRequest.of(pageable, perpage));
		return pagedresult1;
	}

	public int countgse(String gseno) {
		return repo.countgse(gseno);
	}
