...
import com.app.Registration.repository.Service1Repository;

@Service
@Component
public class Service1Service1 implements Iservice {
	@Autowired
	private Service1Repository repo;

	public List<com.app.Registration.model.servicetab> fetchProductList() {
		return repo.findAll();
	}

	public servicetab saveservicetoDB(servicetab service) {
		return repo.save(service);
	}

	public java.util.Optional<servicetab> fetchServiceById(int id) {
		return repo.findById(id);
	}

	public java.util.Optional<servicetab> deleteserviceByIdtest(int id) throws Exception {
		java.util.Optional<servicetab> u = this.repo.findById(id);
		if (u.isPresent())
	...
	public List<servicetab> findPaginatedservice(int pageNo, int perpage) {
		...
		Page<servicetab> pagedresult = repo.findAll(pageable);
	...
	public List<servicetab> findByservicenumber(String seqno, int pageable, int perpage) {
		List<servicetab> pagedresult1 = (List<servicetab>) repo.findByservice1(seqno,
				PageRequest.of(pageable, perpage));
		return pagedresult1;
	}

	public List<servicetab> findByservicesmrdno(String seqno, int pageable, int perpage) {
		List<servicetab> pagedresult1 = (List<servicetab>) repo.findByservicesmrdno1(seqno,
				PageRequest.of(pageable, perpage));
		return pagedresult1;
	}
