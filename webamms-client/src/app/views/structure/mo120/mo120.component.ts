...
import { Mo120Service } from './mo120.service';
...
@Component({
  selector: 'app-mo120',
  templateUrl: './mo120.component.html',
  providers: [
    { provide: CarouselConfig, useValue: { interval: 1500, noPause: false } },
  ],
 
  encapsulation: ViewEncapsulation.None,
  styleUrls: ['./mo120.component.css']
})

export class Mo120Component implements OnInit,OnDestroy {
  acitvities=[];
  start:any=1;
  datagse:any;
  dataspare:any;
...
m1:any;
 ...
 datapage:any;
  ...
  constructor(private _http:HttpClient,
    ...
    private _service:Mo120Service,private _spareservice:SpareserviceService,
    private _gseservice:GseserviceService,private _route:Router,
    private _activatedrouter:ActivatedRoute,private toastr:NotificationService) 
  {
  ...
  ngOnInit(): void {
    ...
    itemsPerPage: this.perpage,
     ...
     this._service.countrecordmo120().subscribe(
      mydata=>{console.log(mydata);
       ...
       this.m1=this.rec1;
        ...
        console.log("The perpage is "+this.perpage);
         ...
         this._service.getdatamo120(0,this.perpage).subscribe(
          (data)=>{console.log(data);
            this.data=data;
            this.mo120=data;
          this.totalItems=this.rec1;
          let activities=this.mo120;
           }/*for first mo120*/
      )/* for first getdata*/
...
handlePageChange(event:any) {
  this.x= event.page;
   ...
   if (this.mode=='searching')
   {
     ...
     this._service.counteruserecordmo120(this.searchname).subscribe(
   ...
   this._service.fetchUserBymo120(this.x-1,parseInt(this.perpage),this.searchname).subscribe(
     (datasear)=>{console.log(datasear);
       this.datapage=datasear;
       this.data=datasear;
     this.rec1=this.searchpages;
     this.mo120=datasear;
     console.log("to test spare"+this.mo120);
     });
   ...
   {
   this._service.getdatamo120(this.x-1,this.perpage).subscribe(
     (data)=>{console.log(data);
       this.data=data;
 ...
 /*--------------------------------*/
   ...
   {
  this._service.fetchUserBymo120(this.m2,parseInt(this.perpage),name).subscribe(
     (datasear)=>{console.log(datasear);
       this.datapage=datasear;
       this.data=datasear;
     this.rec1=this.searchpages;
 ...
 {
   this._service.fetchUserBymo120(this.m2,this.perpage,name).subscribe(
     (datasear)=>{console.log(datasear);
       this.datapage=datasear;
       this.data=datasear;
       this.rec1=this.searchpages;
...
gotodeleteuser(id: number)
  {
    this.mode="deleting";
     ...
     this._service.deleteUserByIdFromRemotemo120(id).subscribe (
       data => {
       ...
       // alert(this.x+this.perpageflag);
        if (this.perpageflag=="true")
        {
          this._service.getdatamo120(this.x-1,parseInt(this.perpage)).subscribe(
            (data)=>{console.log(data);
              this.data=data;
        ...
        {
          this._service.getdatamo120(this.x,3).subscribe(
            (data)=>{console.log(data);
              this.data=data;
            //  this.totalItems=String(100);
            })
        ...
        this.toastr.showWarning("Deletion success", 'Mo120 service');
      },
      error=>{
        this.msg1=error;
         console.log(this.msg1);
         this.toastr.showError(error,'Mo120 service')
      }
    ) 
