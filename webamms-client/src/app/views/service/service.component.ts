import {
  Component, OnInit, ViewEncapsulation, OnDestroy} from "@angular/core";
import { HttpClient } from "@angular/common/http";
import { Router, ActivatedRoute } from "@angular/router";
import { CarouselConfig } from "ngx-bootstrap/carousel";
import { ScriptService } from "./scipt.service.service";
...
export class ServiceComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.service =
      JSON.parse(sessionStorage.getItem("service")) || new Service();
  ...
  ngOnInit(): void {
    ...
    this._service.countrecordservice().subscribe((mydata) => {
      console.log(mydata);
      this.rec1 = mydata;
    ...
    this._service.getdataservice(0, this.perpage).subscribe(
      (data) => {
        console.log(data);
        this.data = data;
        this.service = data;
        this.totalItems = this.rec1;
        let activities = this.service;
      } /*for first service*/
    ); /* for first getdata*/
    ...
    if (this.mode == "searching") {
      ...
      this._service
        ...
        .subscribe((datasear) => {
          ...
          this.data = datasear;
          this.rec1 = this.searchpages;
          this.service = datasear;
          console.log("to test spare" + this.service);
        });
    } /*IF */ else if (this.mode == "normal" || this.mode == "deleting") {
      this._service.getdataservice(this.x - 1, this.perpage).subscribe(
        (data) => {
          console.log(data);
          this.data = data;
  ...
  gotosearchservice(name: any) {
    ...
    this._service.counteruserecordservice(this.searchname).subscribe((recs) => {
      console.log(recs);
      this.searchpages = parseInt(recs);
      console.log("Total items is search=" + this.searchpages);
    });
    ...
    if (this.perpageflag == "true") {
      ...
      this._service.fetchUserByservice(this.m2, this.perpage, name).subscribe(
        (datasear) => {
          console.log(datasear);
          this.datapage = datasear;
          this.data = datasear;
  ...
  gotodeleteuser(id: number) {
    ...
    this._service.deleteUserByIdFromRemoteservice(id).subscribe(
      (data) => {
        ...
        if (this.perpageflag == "true") {
          this._service
            .getdataservice(this.x - 1, parseInt(this.perpage))
            .subscribe((data) => {
              console.log(data);
              this.data = data;
              // this.totalItems=String(100);
            });
        } /* per page true */ else if (this.perpageflag == undefined) {
          this._service.getdataservice(this.x, 3).subscribe((data) => {
            console.log(data);
            this.data = data;
            //  this.totalItems=String(100);
          });
        } /* perpage false */
        this.toastr.showWarning("Deletion success", 'Service');
      },
      (error) => {
        this.msg1 = error;
